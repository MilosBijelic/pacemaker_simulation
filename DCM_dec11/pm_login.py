...
# ---- Init Functions
   ...
   def init_parameter_window(self):
      ...
      self.rb_ctp_D = QRadioButton("D")
      rbg_chambertopulse.addButton(self.rb_ctp_D)

      container = Wrapper((self.rb_ctp_A,self.rb_ctp_V,self.rb_ctp_D))
      layout.addRow("Chamber to Pace: ",container);
      ...
      self.rb_cts_D = QRadioButton("D")
      rbg_chambertosense.addButton(self.rb_cts_D)
      self.rb_cts_none = QRadioButton("None")
      rbg_chambertosense.addButton(self.rb_cts_none)

      container = Wrapper((self.rb_cts_A,self.rb_cts_V,self.rb_cts_D,self.rb_cts_none))
      ...
      # AV delay
      self.sb_avdelay = QSpinBox()
      self.sb_avdelay.setRange(70,300)

      container = Wrapper((self.sb_avdelay, QLabel("(msec)")));
      layout.addRow("AV Delay: ", container)
      
      # PVARP
      self.sb_pvarp = QSpinBox()
      self.sb_pvarp.setRange(150,500)

      container = Wrapper((self.sb_pvarp, QLabel("(msec)")));
      layout.addRow("PVARP: ", container)
      ...
      self.rb_ctp_D.clicked.connect(self.check_chamber_pulse)
      self.rb_cts_A.clicked.connect(self.check_chamber_sense)
      self.rb_cts_V.clicked.connect(self.check_chamber_sense)
      self.rb_cts_D.clicked.connect(self.check_chamber_sense)
...
# ----  Button Functions
   ...
   def check_chamber_pulse(self):
      ...
      elif (self.rb_ctp_D.isChecked()):
         self.rb_cts_D.setChecked(True)
   ...
   def check_chamber_sense(self):
      if (self.rb_cts_A.isChecked()):
         ...
         self.rb_ctp_V.setChecked(True)
      elif (self.rb_cts_D.isChecked()):
         self.rb_ctp_D.setChecked(True)
...
def dataSendLoop(addData_callback):
    ...
    while(not stop):
        ...
        source.data_signal.emit(y[i], y[(i-500)%(len(n))]) 
